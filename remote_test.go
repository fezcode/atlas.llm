...
func TestNormalizeEndpoint(t *testing.T) {
	...
	if got, _ := normalizeEndpoint("gpu-box:9001"); got != "http://gpu-box:9001" {
	...
	if got, _ := normalizeEndpoint("http://host:8080/v1"); got != "http://host:8080" {
	...
	for _, in := range []string{"ftp://host:9", "http://", "://nope"} {
		if got, err := normalizeEndpoint(in); err == nil {
...
func TestRemoteServerUnreachableErrorNamesTheAddress(t *testing.T) {
	...
	_, err := newRemoteServer("http://127.0.0.1:1", "")
	if err == nil {
...
func TestRemoteServerSendsAPIKey(t *testing.T) {
	...
	if _, err := newRemoteServer(srv.URL, "sekrit"); err != nil {
		t.Fatal(err)
	}
...
func TestRemoteModeSkipsLocalRequirements(t *testing.T) {
	...
	if err := saveConfig(Config{CurrentModel: defaultModel, Endpoint: "192.168.1.50:8080"}); err != nil {
		t.Fatal(err)
	}
	if err := requireInference(); err != nil {
...
func TestClientURLs(t *testing.T) {
	got := clientURLs(serveOptions{Bind: "192.168.1.50", Port: 9001})
	if len(got) != 1 || got[0] != "http://192.168.1.50:9001" {
		t.Errorf("explicit bind = %v", got)
	}
	for _, u := range clientURLs(serveOptions{Bind: "0.0.0.0", Port: 8080}) {
...
func TestServeRefusesWhenConfiguredAsClient(t *testing.T) {
	...
	err := runServe(serveOptions{Bind: "0.0.0.0", Port: 8080})
	if err == nil {
...
func TestServeSlotsDivideContextRatherThanMultiplyVRAM(t *testing.T) {
	const ctxN = 16384
	_, base := serveCapacity(ctxN, 0)
	if base != ctxN*kvCacheSlots {
		t.Fatalf("default budget = %d, want %d", base, ctxN*kvCacheSlots)
	}

	for _, want := range []int{2, 4, 8} {
		slots, total := serveCapacity(ctxN, want)
		if slots != want {
	...
	slots, total := serveCapacity(ctxN, 0)
	if slots != kvCacheSlots || total/slots != ctxN {
		t.Errorf("default launch changed: %d slots of %d, want %d of %d",
			slots, total/slots, kvCacheSlots, ctxN)
	}
}

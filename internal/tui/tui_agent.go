...
import (
	...
	// The browser tools register themselves into tools.ToolRegistry from an
	// init, so the package has to be linked in for the agent to be able to
	// call them and for /help tools to list them.
	_ "atlas.llm/internal/browser"
...
func runChatCmd(ctx context.Context, history []engine.ChatMessage, input string) tea.Cmd {
	return func() tea.Msg {
		reply, err := engine.Chat(ctx, history, input)
		if err != nil {
...
func runAgentStepCmd(ctx context.Context, msgs []engine.ChatMsg) tea.Cmd {
	snapshot := append([]engine.ChatMsg(nil), msgs...)
	return func() tea.Msg {
		cfg, _ := config.LoadConfig()
		content, calls, err := engine.RunAgentStep(ctx, snapshot, cfg.MaxTokens)
...
func runToolCmd(call engine.ToolCall) tea.Cmd {
	return func() tea.Msg {
		t, ok := mcpx.LookupTool(call.Function.Name)
		if !ok {
...
func (m *chatModel) handleAgentStep(msg agentStepMsg) tea.Cmd {
	...
	m.agentMsgs = append(m.agentMsgs, engine.ChatMsg{
		Role:      "assistant",
		Content:   msg.content,
	...
	cfgRounds, _ := config.LoadConfig()
	limit := config.ResolveMaxToolRounds(cfgRounds)
	if limit != config.UnlimitedToolRounds && m.stepCount >= limit {
...
func (m *chatModel) dispatchNextTool() tea.Cmd {
	...
	if call.Function.Name == tools.AskUserToolName {
...
func (m *chatModel) appendToolResult(call engine.ToolCall, content string) {
	m.agentMsgs = append(m.agentMsgs, engine.ChatMsg{
		Role:       "tool",
		ToolCallID: call.ID,
...
func (m *chatModel) renderToolTrace(call engine.ToolCall, note string, bad bool) {
	var args map[string]any
	_ = json.Unmarshal([]byte(call.Function.Arguments), &args)
	argSummary := tools.SummarizeToolCallArgs(args)
	style := lipgloss.NewStyle().Foreground(ui.ColDim).Italic(true)
	if bad {
		style = lipgloss.NewStyle().Foreground(ui.ColErr).Italic(true)
	}
...
func (m *chatModel) renderToolResult(_ engine.ToolCall, result string, bad bool) {
	const maxLines = 3
...
func (m *chatModel) renderConfirm() {
	...
	title := ui.BrandStyle.Render("Tool call requires approval") + ui.SysStyle.Render("  (↑/↓ switch · enter confirm · esc deny)")
	sub := lipgloss.NewStyle().Foreground(ui.ColErr).Bold(true).Render("DESTRUCTIVE") +
		"  " + ui.MetaLabelStyle.Render(m.confirmCall.Function.Name)
	argStyle := lipgloss.NewStyle().Foreground(ui.ColMuted)
...
func parseSummarizeArgs(args []string) (cmdops.SummarizeOptions, error) {
	opts := cmdops.SummarizeOptions{
		TargetDir: ".",
		Output:    "SUMMARY.md",
		MaxSize:   cmdops.DefaultSummarizeMaxSize,
	}
	for _, a := range args {
		switch {
...
func runSummarizeCmd(opts cmdops.SummarizeOptions) tea.Cmd {
	return func() tea.Msg {
		err := cmdops.SummarizeDirectory(opts, progressToSysMsg())
...
func runGrepCmd(dir, query string) tea.Cmd {
	return func() tea.Msg {
		hits, err := cmdops.GrepDirectory(dir, query, cmdops.DefaultGrepMaxSize, progressToSysMsg())
...
func (m *chatModel) noteRepeatedCall(call engine.ToolCall) bool {

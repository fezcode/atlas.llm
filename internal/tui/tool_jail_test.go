...
func TestResolveInRootAllowsRootAndSubdirs(t *testing.T) {
	...
	for _, in := range []string{"", ".", "a", "a/b", "./a/b", "a/b/new.txt", root, filepath.Join(root, "a")} {
		got, err := tools.ResolveInRoot(in)
		if err != nil {
...
func TestResolveInRootRejectsEscapes(t *testing.T) {
	...
	for _, in := range []string{
		...
		if got, err := tools.ResolveInRoot(in); err == nil {
...
func TestResolveInRootRejectsSymlinkEscape(t *testing.T) {
	...
	if got, err := tools.ResolveInRoot("escape/secret.txt"); err == nil {
		t.Errorf("symlink escape allowed: %q", got)
	}
	// And the link itself.
	if got, err := tools.ResolveInRoot("escape"); err == nil {
...
func TestResolveInRootErrorIsInformative(t *testing.T) {
	root := jailRoot(t)
	_, err := tools.ResolveInRoot("../nope")
	if err == nil {
...
func TestToolsAreJailed(t *testing.T) {
	...
	if out, err := tools.ToolReadFile(map[string]any{"path": "inside.txt"}); err != nil || out != "hello" {
	...
	cases := []struct {
		...
		{"read_file", func() (string, error) { return tools.ToolReadFile(map[string]any{"path": secret}) }},
		{"list_dir", func() (string, error) { return tools.ToolListDir(map[string]any{"path": outside}) }},
		{"grep", func() (string, error) {
			return tools.ToolGrep(map[string]any{"pattern": "classified", "path": outside})
		}},
		{"write_file", func() (string, error) {
			return tools.ToolWriteFile(map[string]any{"path": filepath.Join(outside, "x.txt"), "content": "x"})
		}},
		{"edit_file", func() (string, error) {
			return tools.ToolEditFile(map[string]any{"path": secret, "old_string": "classified", "new_string": "x"})
		}},
		{"multi_edit", func() (string, error) {
			return tools.ToolMultiEdit(map[string]any{"path": secret,
				"edits": []any{map[string]any{"old_string": "classified", "new_string": "x"}}})
		}},
		{"run_cmd cwd", func() (string, error) {
			return tools.ToolRunCmd(map[string]any{"command": "pwd", "cwd": outside})
		}},
	}
	for _, c := range cases {
...
func TestRunCmdUsesJailedWorkingDirectory(t *testing.T) {
	...
	out, err := tools.ToolRunCmd(map[string]any{"command": pwd})
	if err != nil {
		t.Fatal(err)
	}
	...
	out, err = tools.ToolRunCmd(map[string]any{"command": pwd, "cwd": "sub"})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := tools.ToolRunCmd(map[string]any{"command": pwd, "cwd": "nope"}); err == nil {
...
func TestDisplayPathIsRelative(t *testing.T) {
	root := jailRoot(t)
	if got := tools.DisplayPath(filepath.Join(root, "a", "b.go")); got != filepath.Join("a", "b.go") {
		t.Errorf("displayPath = %q, want a/b.go", got)
	}
	if got := tools.DisplayPath(root); got != "." {
...
func TestRepeatedCallDetection(t *testing.T) {
	m := &chatModel{}
	call := engine.ToolCall{Function: engine.ToolCallFunction{Name: "list_dir", Arguments: `{"path":"internal"}`}}
	...
	other := engine.ToolCall{Function: engine.ToolCallFunction{Name: "list_dir", Arguments: `{"path":"."}`}}

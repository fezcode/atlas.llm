...
func chatModelWithHistory(n int) *chatModel {
	...
	for i := 0; i < n; i++ {
		...
		m.history = append(m.history, engine.ChatMessage{Role: role, Content: "turn " + string(rune('A'+i%26))})
	}
	return m
}
...
func TestApplyCompactionDropsOrphanedToolTurns(t *testing.T) {
	...
	m.agentMsgs = []engine.ChatMsg{{Role: "system", Content: "sys"}}
	for i := 0; i < 6; i++ {
		m.agentMsgs = append(m.agentMsgs, engine.ChatMsg{Role: "user", Content: "q"})
	}
	m.agentMsgs = append(m.agentMsgs,
		engine.ChatMsg{Role: "assistant", ToolCalls: []engine.ToolCall{{ID: "call_1"}}},
		engine.ChatMsg{Role: "tool", ToolCallID: "call_1", Content: "result"},
		engine.ChatMsg{Role: "assistant", Content: "final"},
	)
...
func TestToolResultCapIsSaneRelativeToContext(t *testing.T) {
	const ctx = 16384
	approxTokens := tools.ToolResultSizeLimit / 4
	if pct := approxTokens * 100 / ctx; pct > 15 {
		t.Errorf("one tool result can consume %d%% of a %d-token context "+
			"(%d bytes ≈ %d tokens) — too much", pct, ctx, tools.ToolResultSizeLimit, approxTokens)
	}
}
...
func TestNoThinkingRequestShape(t *testing.T) {
	withKwargs, _ := json.Marshal(engine.ChatRequest{
		Messages:           []engine.ChatMsg{{Role: "user", Content: "hi"}},
		MaxTokens:          128,
	...
	plain, _ := json.Marshal(engine.ChatRequest{
		Messages:  []engine.ChatMsg{{Role: "user", Content: "hi"}},
		MaxTokens: 128,
	})

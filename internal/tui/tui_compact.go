...
func (m *chatModel) handleCompact() tea.Cmd {
	...
	before := engine.GetLastUsage().TotalTokens
	m.busy = true
...
func runCompactCmd(ctx context.Context, transcript string, dropped, before int, wasAgent bool) tea.Cmd {
	return func() tea.Msg {
		summary, err := engine.RunSingleUser(ctx, compactSystemPrompt,
...
func (m *chatModel) applyCompaction(msg compactDoneMsg) {
	...
	if msg.wasAgent && len(m.agentMsgs) > 0 {
		...
		rebuilt := []engine.ChatMsg{m.agentMsgs[0]} // keep the system prompt
		rebuilt = append(rebuilt,
			engine.ChatMsg{Role: "user", Content: primer},
			engine.ChatMsg{Role: "assistant", Content: ack})

...
func TestInfoURLFor(t *testing.T) {
	...
	for _, in := range []string{"http://host", "", "::::"} {
		if got, ok := engine.InfoURLFor(in); ok {
...
func infoServer(t *testing.T, payload any) *httptest.Server {
	...
	mux.HandleFunc(engine.AtlasInfoPath, func(w http.ResponseWriter, r *http.Request) {
		switch v := payload.(type) {
		case string:
			fmt.Fprint(w, v)
		default:
			w.Header().Set("Content-Type", "application/json")
			_ = engine.WriteJSON(w, v)
		}
	})
...
func TestFetchAtlasInfo(t *testing.T) {
	...
	base, ok := engine.EndpointOneBelow(srv.URL)
	if !ok {
		t.Fatalf("could not derive a base from %s", srv.URL)
	}
	got, found := engine.FetchAtlasInfo(srv.Client(), base, "")
	if !found {
...
func TestFetchAtlasInfoRejectsStrangers(t *testing.T) {
	for _, payload := range []any{
		...
		base, _ := engine.EndpointOneBelow(srv.URL)
		if _, ok := engine.FetchAtlasInfo(srv.Client(), base, ""); ok {
...
func TestProbeHealthyWithoutSidecar(t *testing.T) {
	...
	st := engine.ProbeRemote(srv.URL, "")
	if st.State != engine.RemoteHealthy {
		t.Errorf("state = %v, want healthy", st.State)
	}
	if st.HaveInfo {
		t.Error("claimed to have info from a server with no sidecar")
	}
	out := engine.RenderEndpointProbe(srv.URL, st)
...
func TestProbeReportsMissingKey(t *testing.T) {
	...
	if st := engine.ProbeRemote(srv.URL, "k"); st.State != engine.RemoteHealthy {
...
func TestShortNetError(t *testing.T) {
	...
	for in, wantSubstr := range cases {
		got := engine.ShortNetError(errors.New(in))
...
func TestRemoteBadgeIsCacheOnlyAndStateColoured(t *testing.T) {
	withTempHome(t)
	t.Cleanup(engine.ClearRemoteStatus)
	...
	if err := config.SaveConfig(config.Config{CurrentModel: catalog.DefaultModel, Endpoint: "gpu-box:8080"}); err != nil {
		t.Fatal(err)
	}
	for _, state := range []engine.RemoteState{engine.RemoteUnknown, engine.RemoteHealthy, engine.RemoteDegraded, engine.RemoteUnreachable} {
		engine.SetRemoteStatus(engine.RemoteStatus{Endpoint: "http://gpu-box:8080", State: state})
	...
	// looked.
	engine.SetRemoteStatus(engine.RemoteStatus{Endpoint: "http://10.255.255.1:8080", State: engine.RemoteHealthy})
...
func TestRemoteSectionDescribesTheServer(t *testing.T) {
	...
	engine.SetRemoteStatus(engine.RemoteStatus{
		Endpoint: "http://gpu-box:8080",
		State:    engine.RemoteHealthy,
		HaveInfo: true,
		Info: engine.AtlasServerInfo{
			Service: "atlas.llm", Version: "0.31.0", Model: "ministral-3-14b-instruct",
			EngineVariant: "cuda", GPULayers: engine.MaxGPULayers, CtxPerSlot: 8192, Slots: 4,
			UptimeSeconds: 3600, LlamaBuild: "b10375",
		},
	})
	out := engine.RenderRemoteSection("http://gpu-box:8080")
...
func TestRemoteSectionWhenUnreachable(t *testing.T) {
	...
	engine.SetRemoteStatus(engine.RemoteStatus{
		Endpoint: "http://gpu-box:8080",
		State:    engine.RemoteUnreachable,
...
func TestFormatUptime(t *testing.T) {
	...
	for secs, want := range cases {
		if got := engine.FormatUptime(secs); got != want {

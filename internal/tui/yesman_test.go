...
func TestYesmanBypassesConfirmForDestructiveOnly(t *testing.T) {
	m := newYesmanModel()
	m.agentMsgs = []engine.ChatMsg{{Role: "system", Content: "x"}}

	// Safe tool: never prompts, regardless.
	m.pendingCalls = []engine.ToolCall{{ID: "1", Function: engine.ToolCallFunction{Name: "list_dir", Arguments: `{"path":"."}`}}}
	m.dispatchNextTool()
	...
	m.picking = ""
	m.pendingCalls = []engine.ToolCall{{ID: "2", Function: engine.ToolCallFunction{Name: "run_cmd", Arguments: `{"command":"echo hi"}`}}}
	m.dispatchNextTool()
	if m.picking != "tool_confirm" {
	...
	m.confirmCall = nil
	m.yesman = true
	m.pendingCalls = []engine.ToolCall{{ID: "3", Function: engine.ToolCallFunction{Name: "run_cmd", Arguments: `{"command":"echo hi"}`}}}
	m.dispatchNextTool()
	if m.picking == "tool_confirm" {
...
func TestYesmanCallsAreStillAnnounced(t *testing.T) {
	...
	m.yesman = true
	m.agentMsgs = []engine.ChatMsg{{Role: "system", Content: "x"}}
	m.pendingCalls = []engine.ToolCall{{ID: "1", Function: engine.ToolCallFunction{Name: "write_file", Arguments: `{"path":"x","content":"y"}`}}}
	m.dispatchNextTool()

...
func TestLayersThatFit(t *testing.T) {
	...
	for _, tc := range cases {
		got, ok := engine.LayersThatFit(tc.weights, tc.kv, tc.blocks, tc.used, tc.tot)
		if ok != tc.wantOK {
	...
	n, ok := engine.LayersThatFit(15900*1024*1024, 2*gib, 48, 300, 16303)
	if !ok || n <= 0 || n >= 48 {
...
func TestLayersThatFitReservesHeadroom(t *testing.T) {
	...
	n, ok := engine.LayersThatFit(16303*1024*1024, 0, 40, 0, 16303)
	if !ok {
...
func TestMaxTokensCeilingFollowsTheRemote(t *testing.T) {
	...
	local := config.Config{CurrentModel: catalog.DefaultModel, CtxSize: 16384}
	if err := config.SaveConfig(local); err != nil {
		t.Fatal(err)
	}
	localCeiling := engine.MaxTokensCeiling(local)

	remote := config.Config{CurrentModel: catalog.DefaultModel, CtxSize: 16384, Endpoint: "gpu-box:8080"}
	if err := config.SaveConfig(remote); err != nil {
		t.Fatal(err)
	}
	engine.SetRemoteStatus(engine.RemoteStatus{
		Endpoint: "http://gpu-box:8080", State: engine.RemoteHealthy, HaveInfo: true,
		Info: engine.AtlasServerInfo{Service: "atlas.llm", CtxPerSlot: 8192},
	})
	remoteCeiling := engine.MaxTokensCeiling(remote)
...
func TestGPUSectionEmptyWithoutGPU(t *testing.T) {
	withTempHome(t)
	noGPU(t)
	if got := engine.RenderGPUSection(config.Config{CurrentModel: catalog.DefaultModel}); got != "" {
...
func TestGPUSectionDistinguishesExplicitFromEstimated(t *testing.T) {
	...
	twenty := 20
	out := engine.RenderGPUSection(config.Config{CurrentModel: catalog.DefaultModel, GPULayers: &twenty})

...
func (m *chatModel) applyModelSelection(target catalog.Model) {
	...
	if !config.IsModelDownloaded(target) {
...
func (m *chatModel) renderPicker() {
	title := ui.BrandStyle.Render("Select a model") + ui.SysStyle.Render("  (↑/↓ move · enter select · esc cancel)")
	...
	for i, mm := range m.pickerItems {
		...
		status := ui.SysStyle.Render("not downloaded")
		if config.IsModelDownloaded(mm) {
			status = lipgloss.NewStyle().Foreground(ui.ColAssistant).Render("downloaded")
		}
		row := fmt.Sprintf("%s%-28s  %s%s%s", marker, mm.Name,
			ui.MetaLabelStyle.Render(mm.Size), dot, status)
		if note := engine.ModelResourceNote(mm); note != "" {
			row += dot + ui.SysStyle.Render(note)
		}
		if i == m.pickerIdx {

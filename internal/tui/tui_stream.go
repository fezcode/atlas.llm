...
// reply.
func runChatStreamCmd(ctx context.Context, history []engine.ChatMessage, input string) tea.Cmd {
	snapshot := append([]engine.ChatMessage(nil), history...)
	return func() tea.Msg {
		reply, err := engine.ChatStream(ctx, snapshot, input, func(d engine.StreamDelta) {
			if program == nil {
				return
			}
	...
	if msg.content == "" && msg.reasoning != "" {
		...
		if m.streamBuf == "" {
			if m.streamShowThink {
				m.rendered[m.streamIdx] = ui.ThinkStyle.Render(m.streamThink) + streamCursor
			} else {
				m.rendered[m.streamIdx] = ui.SysStyle.Render(
					fmt.Sprintf("thinking… (%s of reasoning so far)", engine.FormatBytes(int64(len(m.streamThink)))))
			}
			m.refresh()
		}
		return
...
func (m *chatModel) sealThinkBlock() {
	...
	m.rendered[m.streamIdx] = ui.ThinkStyle.Render(m.streamThink)
	m.rendered = append(m.rendered, "", "")
	m.streamIdx = len(m.rendered) - 1
}

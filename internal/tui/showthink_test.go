...
func showThinkModel(t *testing.T, show bool) *chatModel {
	t.Helper()
	withTempHome(t)
	if err := config.SaveConfig(config.Config{CurrentModel: catalog.DefaultModel,
		MaxTokens: config.DefaultMaxTokens, ShowThinking: show}); err != nil {
		t.Fatal(err)
	}
...
func TestShowThinkingProfileRoundTrip(t *testing.T) {
	withTempHome(t)
	want := config.Config{CurrentModel: catalog.DefaultModel, MaxTokens: 2048, ShowThinking: true}
	if err := config.SaveProfile("unittest-think", want); err != nil {
		t.Fatal(err)
	}
	got, err := config.LoadProfile("unittest-think")
	if err != nil {
		t.Fatal(err)
	}
	if !got.ShowThinking {

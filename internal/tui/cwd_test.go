...
func TestLiftLeadingCd(t *testing.T) {
	...
	for _, tt := range tests {
		gotCmd, gotCwd, gotLifted := tools.LiftLeadingCd(tt.cmd, tt.cwd)
...
func TestRunCmdLiftsCdAndExplains(t *testing.T) {
	...
	out, err := tools.ToolRunCmd(map[string]any{"command": "cd sub && " + list})
	if err != nil {
	...
	if _, err := tools.ToolRunCmd(map[string]any{"command": "cd .. && " + list}); err == nil {
...
func TestReasoningSetting(t *testing.T) {
	...
	if config.ReasoningEnabledFor(config.Config{}, false) {
		t.Error("auto should disable reasoning for plain chat")
	}
	if !config.ReasoningEnabledFor(config.Config{}, true) {
		t.Error("auto should keep reasoning for agentic turns")
	}
	// Explicit settings override both.
	if !config.ReasoningEnabledFor(config.Config{Reasoning: "on"}, false) {
		t.Error("on should force reasoning even for chat")
	}
	if config.ReasoningEnabledFor(config.Config{Reasoning: "off"}, true) {
		t.Error("off should suppress reasoning even for agentic turns")
	}
	for _, v := range []string{"off", "OFF", " off "} {
		if config.ReasoningEnabled(config.Config{Reasoning: v}) {
			t.Errorf("%q should disable reasoning", v)
		}
	}
	if !strings.Contains(config.ReasoningDisplay(config.Config{Reasoning: "off"}), "off") {
...
func TestReasoningRoundTrips(t *testing.T) {
	withTempHome(t)
	if err := config.SaveConfig(config.Config{CurrentModel: catalog.DefaultModel, Reasoning: "off"}); err != nil {
		t.Fatal(err)
	}
	...
	if got.Reasoning != "off" || config.ReasoningEnabled(got) {
		t.Errorf("reasoning = %q, enabled=%v", got.Reasoning, config.ReasoningEnabled(got))
	}
}

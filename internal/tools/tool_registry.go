...
// extraTools, when non-nil, contributes tools from beyond the built-in
// registry. The MCP layer installs it at startup; it is a hook, not a direct
// call, so that the tool registry does not depend on the MCP client.
var ExtraTools func() []Tool

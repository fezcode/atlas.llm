...
// session has to pay.
func MemoryDisplay(cfg config.Config) string {
	var parts []string
	...
	if m, err := config.CurrentModel(); err == nil && config.IsModelDownloaded(m) {
		if p, err := config.ModelPath(m); err == nil {
...
func ModelFit(m catalog.Model) (share float64, fit Fit) {
	total, ok := SystemRAM()
	if !ok || total <= 0 {
		return 0, FitUnknown
	}
...
func ModelResourceNote(m catalog.Model) string {
...
// actually 3.11GB).
func ModelSizeBytes(m catalog.Model) int64 {
	if p, err := config.ModelPath(m); err == nil {
		if size, err := statSize(p); err == nil && size > 0 {
			return size
		}
	}
	return config.ParseModelSize(m.Size)
}
...
func modelMemoryEstimate(m catalog.Model, ctx int, cfg config.Config) (weights, kv int64, ok bool) {
...
func RenderMemSegment() string {
	...
	label := ui.MetaLabelStyle.Render("mem ")
	value := ui.MetaValueStyle.Render(FormatBytes(rss))
	...
	switch {
	case pct >= 80:
		style = lipgloss.NewStyle().Foreground(ui.ColErr).Bold(true)
	case pct >= 60:
		style = lipgloss.NewStyle().Foreground(ui.ColBusy).Bold(true)
	}

...
func TestServerIdentityIgnoresResolvedLayers(t *testing.T) {
	baseTuning := TuningFingerprint(config.Config{})
	// A non-nil cmd is what makes a server local rather than remote; no
	// process is started for it.
	s := &llamaServer{
		Cmd:   &exec.Cmd{},
		Model: catalog.Model{Name: "qwen3.5-9b"}, CtxN: 16384, askedCtx: 16384,
	...
	if serverMatches(s, "qwen3.5-9b", 16384, gpuSettingAuto, TuningFingerprint(config.Config{KVOffload: "off"})) {
	...
	remote := &llamaServer{Model: catalog.Model{Name: "qwen3.5-9b"}, CtxN: 16384, askedCtx: 16384,

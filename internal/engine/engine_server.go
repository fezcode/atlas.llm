...
func gpuLayersSetting(cfg config.Config) int {
...
func EnsureServer() (*llamaServer, error) {
	...
	if cfgEndpoint, key := config.RemoteEndpoint(); cfgEndpoint != "" {
...
func startLlamaServer(m catalog.Model) (*llamaServer, error) {
...
// address for `--serve`.
func startLlamaServerWith(m catalog.Model, opts ServeOptions) (*llamaServer, error) {
...
func baseOffload(cfg config.Config, off OffloadPlan) OffloadPlan {
	if off.CPUMoE == 0 {
		return off
	}
...
func launchLlamaServer(bin, modelPath string, m catalog.Model, ngl, ctxN int, optimized bool) (*llamaServer, error) {
	return launchLlamaServerOn(bin, modelPath, m, OffloadPlan{NGL: ngl}, ctxN, optimized, ServeOptions{}, config.Config{})
}
...
func BuildServerArgs(modelPath string, opts ServeOptions, port int, off OffloadPlan, ctxN int, optimized bool, cfg config.Config) ([]string, int, int) {
	slots := 1
...
func resolveCacheTypeV(cfg config.Config) string {
	if cfg.CacheTypeV != "" {
...
func launchLlamaServerOn(bin, modelPath string, m catalog.Model, off OffloadPlan, ctxN int, optimized bool, opts ServeOptions, cfg config.Config) (*llamaServer, error) {
	port := opts.Port
	if port == 0 {
		var err error
...
func NewRemoteServer(endpoint, apiKey string) (*llamaServer, error) {
	...
	s.Model = catalog.Model{Name: remoteModelName(s), Size: "remote"}
...
// setting applied.
func (s *llamaServer) chatCompleteRespectingReasoning(ctx context.Context, msgs []ChatMsg, maxTokens int, cfg config.Config) (string, error) {
	content, _, err := s.chatCompleteCore(ctx, msgs, maxTokens, nil, !config.ReasoningEnabled(cfg))
	return content, err
}

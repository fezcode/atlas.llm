...
import (
	...
	"atlas.llm/internal/tui"
)

// Version is stamped in by the build (-X main.Version). Everything below
// main reads it through buildinfo, which main seeds on the first line of
// main() — a package cannot import main.
var Version = "dev"
...
func main() {
	...
	flag.Int64Var(&maxSizeFlag, "max-size", cmdops.DefaultGrepMaxSize, "")
	...
	flag.IntVar(&portFlag, "port", config.DefaultServePort, "")
	flag.IntVar(&slotsFlag, "slots", engine.DefaultServeSlots, "")
	...
	if serveFlag {
		...
		_, closeLog, logErr := logging.SetupLogging()
		if logErr == nil {
			defer closeLog()
		}
		if err := engine.RunServe(engine.ServeOptions{
			Bind:   bindFlag,
			Port:   portFlag,
			Slots:  slotsFlag,
			APIKey: apiKeyFlag,
	...
	if clearLogsFlag {
		if err := logging.ClearLogs(); err != nil {
	...
	if installProfilesFlag {
		if err := config.InstallProfiles(os.Stdout); err != nil {
	...
	case chatFlagSet:
		...
		reply, err := engine.Chat(context.Background(), nil, prompt)
		if err != nil {
			fmt.Fprintf(os.Stderr, "chat: %v\n", err)
			os.Exit(1)
		}
	...
	case grepFlag != "":
		...
		hits, err := cmdops.GrepDirectory(targetDir, grepFlag, maxSizeFlag, nil)
		if err != nil {
			fmt.Fprintf(os.Stderr, "grep: %v\n", err)
			os.Exit(1)
		}
		fmt.Println(cmdops.FormatGrepHits(hits))
	...
	case summarizeFlag:
		...
		if maxSizeFlag == cmdops.DefaultGrepMaxSize {
			summarizeMax = cmdops.DefaultSummarizeMaxSize
		}
		...
		if err := cmdops.SummarizeDirectory(opts, nil); err != nil {
			fmt.Fprintf(os.Stderr, "summarize: %v\n", err)
			os.Exit(1)
		}
	...
	case dumpFlag:
		...
		err := cmdops.RunDump(cmdops.DumpOptions{
			TargetDir: targetDir,
			Output:    outputFlag,
			Exclude:   excludes,

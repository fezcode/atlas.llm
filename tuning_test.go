...
func applyTuning(t *testing.T, cfg *Config, key, val string) error {
	t.Helper()
...
func TestTuningSettingsRegistered(t *testing.T) {
	...
	for _, k := range keys {
		...
		if got := s.Value(Config{}); got == "" {
			t.Errorf("%s renders an empty value on a zero config", k)
		}
		if s.Detail == nil || s.Detail(Config{}) == "" {
...
func TestApplyTuningSettings(t *testing.T) {
	cases := []struct {
		...
		{"kv_offload", "off", false, func(c Config) bool { return c.KVOffload == "off" }},
		{"kv_offload", "on", false, func(c Config) bool { return c.KVOffload == "" }},
		{"kv_offload", "maybe", true, nil},
		{"flash_attn", "off", false, func(c Config) bool { return c.FlashAttn == "off" }},
		{"flash_attn", "auto", false, func(c Config) bool { return c.FlashAttn == "" }},
		{"flash_attn", "fast", true, nil},
		{"cache_type_k", "q4_0", false, func(c Config) bool { return c.CacheTypeK == "q4_0" }},
		{"cache_type_k", "auto", false, func(c Config) bool { return c.CacheTypeK == "" }},
		{"cache_type_k", "q9_9", true, nil},
		{"cache_type_v", "f16", false, func(c Config) bool { return c.CacheTypeV == "f16" }},
		{"threads", "4", false, func(c Config) bool { return c.Threads == 4 }},
		{"threads", "auto", false, func(c Config) bool { return c.Threads == 0 }},
		{"threads", "0", true, nil},
		{"threads", "-2", true, nil},
		{"batch_size", "1024", false, func(c Config) bool { return c.BatchSize == 1024 }},
		{"batch_size", "nope", true, nil},
		{"ubatch_size", "256", false, func(c Config) bool { return c.UBatchSize == 256 }},
		{"parallel", "4", false, func(c Config) bool { return c.Parallel == 4 }},
		{"parallel", "0", true, nil},
		{"cache_reuse", "off", false, func(c Config) bool { return c.CacheReuse != nil && *c.CacheReuse == 0 }},
		{"cache_reuse", "512", false, func(c Config) bool { return c.CacheReuse != nil && *c.CacheReuse == 512 }},
		{"cache_reuse", "auto", false, func(c Config) bool { return c.CacheReuse == nil }},
		{"mmap", "off", false, func(c Config) bool { return c.Mmap == "off" }},
		{"mmap", "on", false, func(c Config) bool { return c.Mmap == "" }},
		{"mlock", "on", false, func(c Config) bool { return c.Mlock == "on" }},
		{"mlock", "off", false, func(c Config) bool { return c.Mlock == "" }},
		{"seed", "42", false, func(c Config) bool { return c.Seed != nil && *c.Seed == 42 }},
		{"seed", "auto", false, func(c Config) bool { return c.Seed == nil }},
		{"seed", "-3", true, nil},
		{"temperature", "0.7", false, func(c Config) bool { return c.Temperature != nil && *c.Temperature == 0.7 }},
		{"temperature", "auto", false, func(c Config) bool { return c.Temperature == nil }},
		{"temperature", "3.5", true, nil},
		{"override_tensor", `exps=CPU`, false, func(c Config) bool { return c.OverrideTensor == "exps=CPU" }},
		{"override_tensor", "off", false, func(c Config) bool { return c.OverrideTensor == "" }},
		{"context_shift", "on", false, func(c Config) bool { return c.ContextShift == "on" }},
		{"context_shift", "off", false, func(c Config) bool { return c.ContextShift == "" }},
	}
...
func TestBuildServerArgsDefaultsOptimized(t *testing.T) {
	args, slots, serverCtx := buildServerArgs("model.gguf", serveOptions{}, 8080,
		offloadPlan{NGL: maxGPULayers}, 16384, true, Config{})
	if slots != kvCacheSlots {
		t.Errorf("slots = %d, want %d", slots, kvCacheSlots)
	}
	if serverCtx != 16384*kvCacheSlots {
		t.Errorf("serverCtx = %d, want %d", serverCtx, 16384*kvCacheSlots)
	}
	for flag, want := range map[string]string{
		"-fa":            "on",
		"--cache-type-k": "q8_0",
		"--cache-type-v": "q8_0",
		"--parallel":     fmt.Sprintf("%d", kvCacheSlots),
		"--cache-reuse":  "256",
		"-c":             fmt.Sprintf("%d", 16384*kvCacheSlots),
		"-ngl":           fmt.Sprintf("%d", maxGPULayers),
	} {
...
func TestBuildServerArgsDefaultsFallback(t *testing.T) {
	args, slots, serverCtx := buildServerArgs("model.gguf", serveOptions{}, 8080,
		offloadPlan{NGL: 0}, 16384, false, Config{})
	if slots != 1 {
...
func TestBuildServerArgsTuning(t *testing.T) {
	...
	cases := []struct {
		...
		{"kv_offload off", Config{KVOffload: "off"}, "--no-kv-offload", ""},
		{"mmap off", Config{Mmap: "off"}, "--no-mmap", ""},
		{"mlock on", Config{Mlock: "on"}, "--mlock", ""},
		{"context_shift on", Config{ContextShift: "on"}, "--context-shift", ""},
		{"seed", Config{Seed: &seed}, "--seed", "7"},
		{"threads", Config{Threads: 2}, "-t", "2"},
		{"batch", Config{BatchSize: 4096}, "-b", "4096"},
		{"ubatch", Config{UBatchSize: 128}, "-ub", "128"},
		{"override_tensor", Config{OverrideTensor: "exps=CPU"}, "-ot", "exps=CPU"},
		{"cache_type_k", Config{CacheTypeK: "q4_0"}, "--cache-type-k", "q4_0"},
		{"cache_type_v", Config{CacheTypeV: "f16"}, "--cache-type-v", "f16"},
		{"flash_attn off", Config{FlashAttn: "off"}, "-fa", "off"},
		{"cache_reuse", Config{CacheReuse: &i512}, "--cache-reuse", "512"},
	}
	for _, tc := range cases {
		args, _, _ := buildServerArgs("model.gguf", serveOptions{}, 8080,
			offloadPlan{NGL: 0}, 16384, true, tc.cfg)
	...
	args, slots, serverCtx := buildServerArgs("model.gguf", serveOptions{}, 8080,
		offloadPlan{NGL: 0}, 16384, true, Config{Parallel: 4})
	if slots != 4 {
		t.Errorf("parallel slots = %d, want 4", slots)
	}
	if serverCtx != 16384*kvCacheSlots {
		t.Errorf("parallel serverCtx = %d, want the fixed budget %d", serverCtx, 16384*kvCacheSlots)
	}
	...
	args, _, _ = buildServerArgs("model.gguf", serveOptions{}, 8080,
		offloadPlan{NGL: 0}, 16384, false, Config{FlashAttn: "on", KVOffload: "off"})
...
func TestTuningFingerprint(t *testing.T) {
	...
	if got := tuningFingerprint(Config{Temperature: &temp}); got != base {
	...
	zero := 0
	if tuningFingerprint(Config{CacheReuse: &zero}) == base {
...
func TestRemoteDecidesTuning(t *testing.T) {
	for _, k := range []string{"kv_offload", "flash_attn", "cache_type_k",
		...
		"context_shift"} {
		if !remoteDecidesSetting(k) {
			t.Errorf("%s should be flagged as remote-decided", k)
		}
	}
	if remoteDecidesSetting("temperature") {
...
func TestResolveTemperature(t *testing.T) {
	if got := resolveTemperature(Config{}); got != 0.2 {
		t.Errorf("default temperature = %v, want 0.2", got)
	}
	temp := 0.8
	if got := resolveTemperature(Config{Temperature: &temp}); got != 0.8 {
...
func TestVRAMKVCharge(t *testing.T) {
	kv := int64(2 << 30)
	if got := vramKVCharge(Config{}, kv); got != kv {
		t.Errorf("default charge = %d, want %d", got, kv)
	}
	if got := vramKVCharge(Config{KVOffload: "off"}, kv); got != 0 {
...
func TestKVEstimateFollowsCacheType(t *testing.T) {
	meta := ggufMeta{BlockCount: 32, HeadCount: 32, HeadCountKV: 8,
		KeyLength: 128, EmbeddingLength: 4096}
	def := meta.kvCacheBytes(16384)
	f16 := int64(float64(def) / kvCacheElemBytes * 2)
...
func TestTuningValueRendering(t *testing.T) {
	...
	temp := 0.6
	i64 := 64
	cfg := Config{KVOffload: "off", FlashAttn: "off", CacheTypeK: "q4_0",

...
func (m *chatModel) startAMA(call ToolCall) tea.Cmd {
	if !amaOn.Load() {
...
func (m *chatModel) renderAMA() {
	...
	spec := m.amaSpec
	title := brandStyle.Render("The agent is asking") + sysStyle.Render("  ("+spec.amaHint()+")")
...
func (m *chatModel) resolveAMA(submit bool) tea.Cmd {
	...
	answer := formatAMASelection(chosen)
...
func (m *chatModel) handleAMA(args []string) {
	...
	if len(args) == 0 {
		m.pushSystem(fmt.Sprintf("ama = %s  (use `/ama on` to let the agent ask you questions with interactive lists)", onOff(amaOn.Load())))
		return
	}
